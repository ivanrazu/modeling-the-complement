...
% Neutrophilsig
dN_dt = Kn * (Knn * C5a * N + Kna * A + Knd * D) * F_aif / ( 1 + (Knn * C5a * N + Kna * A + Knd * D) * F_aif )  - mun *( 1 +  (dnc * Ac + dns *As+ dnh * Ah )  * F_aif  * 1/(1+C5a/C5astar) ) * N;

% Macrophages
dM_dt = Km * (Kmm * C5a * M + Kma * A + Kmd * D) * F_aif / ( 1 + (Kmm * C5a * M + Kma * A + Kmd * D) * F_aif ) - mum * ( 1 + (dmc * Ac + dms * As ) * F_aif ) * M;
